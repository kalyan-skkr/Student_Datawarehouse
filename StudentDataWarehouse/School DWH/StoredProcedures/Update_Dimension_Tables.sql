...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			INSERT INTO DIMGP
			SELECT SG.STUDENTID, SG.COURSEID, SG.GP
			FROM STAGEGP SG
			WHERE SG.STUDENTID = @CUR_STUD AND SG.COURSEID NOT IN (SELECT COURSEID FROM DIMGP WHERE STUDENTID = @CUR_STUD)
	...
	INSERT INTO DIMCOURSE (COURSEID, SCHOOLID, COURSENAME)
	SELECT SC.COURSEID, SC.SCHOOLID, SC.COURSENAME
	FROM STAGECOURSE SC
	LEFT JOIN DIMCOURSE D ON D.COURSEID = SC.COURSEID
	WHERE D.COURSEID = NULL
